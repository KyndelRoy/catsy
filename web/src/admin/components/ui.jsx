...
export function Badge({ variant, children }) {
  ...
  return (
    <span style={{
      ...
      borderRadius: 9999, fontSize: 13.75, fontWeight: 600,
...
export function Btn({ variant = "outline", onClick, children, style }) {
  ...
  const variants = {
    primary: { background: C.fg, color: "hsl(240,5.9%,10%)", border: "none", padding: "7px 14px", fontSize: 16.25 },
    ...
    <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, overflow: "hidden", ...style }}>
      {title && (
        <div style={{ padding: "14px 20px", borderBottom: `1px solid hsl(240,3.7%,12%)`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span style={{ fontSize: 16.25, fontWeight: 600, color: "hsl(0,0%,90%)" }}>{title}</span>
          {action && <span style={{ fontSize: 15, color: C.muted, cursor: "pointer", fontWeight: 500 }} onClick={onAction}>{action}</span>}
        </div>
      )}
      {children}
    </div>
...
export function PageHeader({ title, subtitle, action, onAction }) {
  return (
    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 20 }}>
      <div>
        <h2 style={{ fontSize: 27.5, fontWeight: 700, color: C.fg, letterSpacing: "-0.02em" }}>{title}</h2>
        <p style={{ fontSize: 15, color: C.muted, marginTop: 2 }}>{subtitle}</p>
      </div>
...
export function Select({ value, onChange, children, style }) {
  return (
    <select value={value} onChange={onChange} style={{
      ...
      padding: "7px 12px", fontSize: 16.25, color: C.fg, outline: "none",
    ...
    <div onClick={(e) => e.target === e.currentTarget && onClose()}
      ...
      <div style={{ background: "hsl(240,5.9%,7%)", border: `1px solid ${C.borderMd}`, borderRadius: 12, width: 520, maxWidth: "95vw", maxHeight: "90vh", overflowY: "auto", boxShadow: "0 25px 60px rgba(0,0,0,0.6)" }}>
        <div style={{ padding: "20px 24px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <h3 style={{ fontSize: 20, fontWeight: 600, color: C.fg, letterSpacing: "-0.01em" }}>{title}</h3>
...
export function FormGroup({ label, children }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      <label style={{ fontSize: 13.75, textTransform: "uppercase", letterSpacing: "0.08em", color: C.muted, fontWeight: 600 }}>{label}</label>
      {children}
    </div>
  );
}
...
export function StatCard({ label, value, sub, subColor, accentColor, icon }) {
  return (
    <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 20, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ marginBottom: 12, fontSize: 22.5, color: accentColor }}>{icon}</div>
      <div style={{ fontSize: 35, fontWeight: 700, letterSpacing: "-0.03em", color: C.fg }}>{value}</div>
      <div style={{ fontSize: 15, color: C.muted, marginTop: 4 }}>{label}</div>
      <div style={{ fontSize: 13.75, color: subColor || C.muted, fontWeight: 500, marginTop: 8 }}>{sub}</div>
    </div>
  );
}
...
export function TH({ children, style }) {
  return (
    <th style={{
      padding: "10px 16px", textAlign: "left", fontSize: 12.5,
...
export function TD({ children, style, last }) {
  return (
    <td style={{
      padding: "12px 16px", fontSize: 16.25, color: "hsl(0,0%,90%)",
